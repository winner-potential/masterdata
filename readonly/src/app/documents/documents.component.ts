...
export class DocumentsComponent implements OnInit {
  ...
  constructor(private active: ActivatedRoute, private api: ApiService, private router: Router, private actions: ActionsService, public snackBar: MatSnackBar) {}
  ...
  ngOnInit() {
    ...
    this.active.params.subscribe(value => {
      ...
      this.api.getDocumentTemplates().subscribe((temps: Array<object>) => {
        ...
        if (this.search) {
          this.api.findDocuments(this.search).subscribe((documents: Array<object>) => this.import(documents), (err) => this.loadError(err));
        } else {
  ...
  import(documents: Array<object>) {
    ...
    this.documents.sort((a:any, b:any) => {
      return this.typeName(a.template) + "_" + a.name < this.typeName(b.template) + "_" + b.name ? -1 : 1;
    });
  }
